...
<script>
  ...
  var firebaseAppTemp = firebase.initializeApp(config, 'temp');
</script>
...
function doGoogleLogin() {
	console.log(firebaseAppTemp.auth())
	console.log(firebaseAppTemp.auth().currentUser)
	if (!firebaseAppTemp.auth().currentUser) {
		var provider = new firebase.auth.GoogleAuthProvider();
		provider.addScope('https://www.googleapis.com/auth/plus.login');
		//firebaseAppTemp.auth().setPersistence(firebaseAppTemp.auth.Auth.Persistence.SESSION);
		firebaseAppTemp.auth().setPersistence(firebase.auth.Auth.Persistence.SESSION);
		firebaseAppTemp.auth().signInWithPopup(provider).then(function(result) {
		...
		firebaseAppTemp.auth().signOut();
	}
	document.getElementById('quickstart-sign-ing').disabled = true;
}
...
function doGoogleLogout() {
	...
	firebaseAppTemp.auth().signOut().then(function() {
